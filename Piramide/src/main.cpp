...
void GeraMalhaPiramide(GLuint resolucao, std::vector<Vertice> &Vertices
	...
	glm::vec3 ponto_a {1.0f,  1.0f, 1.0f};
	glm::vec3 ponto_b {1.0f, -1.0f, 1.0f};
	glm::vec3 ponto_c {-1.0f,  1.0f, 1.0f};
	glm::vec3 ponto_d {1.0f,  -1.0f, 1.0f};
	glm::vec3 ponto_e {0.0f,  0.0f, -1.0f};
	

	Vertices = {
		// vertices da base
		{ponto_a, {GREEN}, {0.0f, 0.0f}},
		{ponto_b, {GREEN}, {1.0f, 0.0f}},
		{ponto_c, {GREEN}, {1.0f, 1.0f}},
		{ponto_d, {GREEN}, {0.0f, 1.0f}},

		// vertices do topo
		{ponto_e, {BLACK}, {0.0f, 0.0f}},
	};
...
GLuint CarregaPiramide(GLuint &TotalVertices, GLuint &TotalIndices) {

	std::vector<Vertice> Vertices;
	std::vector<glm::ivec3> Triangulos;
	//Depois testa com 100
	GeraMalhaPiramide(50, Vertices, Triangulos);
...
int main() {
	...
	GLuint EsferaVAO = CarregaPiramide(qtdVerticesCubo, qtdIndicesCubo);
	std::cout << "Total de vertices da Piramide: " << qtdVerticesCubo << std::endl;
	std::cout << "Total de indices da Piramide" << qtdIndicesCubo << std::endl;
	...
	glClearColor(0.7f, 0.7f, 0.7f, 0.4f);

	//Tempo de atualizado dos frames
	double  TempoAtualizadoFrameAnterior = glfwGetTime();

	
	//zbuffer (teste de profundidade)
	glEnable(GL_DEPTH_TEST);

	// rENDERIZA O QUE ESTA PROXIMO
	glDepthFunc(GL_LESS);
	...
	while (!glfwWindowShouldClose(window)) {
		...
		//glDrawArrays(GL_POINTS,0, qtdVerticesCubo);
